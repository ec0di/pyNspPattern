...
def master_problem_instance(n_days, n_work_shifts, nurse_df, roster_indices, binary_plans, demand, t_max_sec, solver_id='GLOP'):  # solver_id='CBC' for MIP or GLOP for LP
    ...
    obj = create_objective(nurse_df, roster_indices, solver, z)
...
@timer()
def create_objective(nurse_df, roster_indices, solver, z):
    obj = solver.Sum([z[nurse_type, roster_idx] * total_cost for nurse_type in nurse_df.nurseType.values
                      for roster_idx, total_cost in roster_indices[nurse_type]
